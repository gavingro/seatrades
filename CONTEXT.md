...
- **Suspected** — a pressure signal that likely contributed but needs global reasoning to prove; advisory, ranked, never the single answer. Shipped behind **conservative placeholder thresholds** (in `config`) that err toward silence — a hint never fires on comfortably-feasible input; research spike #115 replaces the numbers without reshaping the checks.
