...
- **Infeasible** — a hard constraint provably cannot be met; no schedule exists. A post-mortem then explains *which* rule broke — see §Infeasibility Diagnosis below (shipped in #78; PRD `docs/prd/infeasibility-diagnosis.md`).
...
The plain-language post-mortem of *why* an infeasible solve produced no schedule. A **pure** module (`diagnostics.py`) over the joined domain data + the validated relationships + only the config knobs each check needs (`campers_max`, `campers_min`, `max_seatrades_per_fleet`, `max_cabin_share_per_seatrade`, `force_same_fleet_all_week`) — no solver, Streamlit, or session state. It runs **only** on `INFEASIBLE` (never pre-solve, never on `OPTIMAL`/`TIMEOUT`), via the single call site `solver.diagnose_infeasibility`, and returns a **ranked list of `Finding`s** carried on `AssignmentSolution.findings`: proven causes first (most-certain), then suspected pressure hints below them. Each `Finding` has a **tier**, a plain-language **cause** — pointing at the specific campers (by `(cabin, name)`) or seatrades when a subset is implicated, or at the over-subscribed cohort when the whole roster is — and a **named fix**. The UI *prepends* proven causes above the retained generic failure copy and renders suspected hints beneath them under a "possible contributing pressures (not certain)" header, so a maybe is never mistaken for the answer; when none fire, an honest "couldn't identify the exact cause" replaces them — never worse than today.
...
- **Suspected** — a pressure signal that likely contributed but needs global reasoning to prove; advisory, never the single answer. Sits behind **conservative thresholds** (in `config`) that err toward silence — a hint never fires on comfortably-feasible input. Research spike #115 replaced #114's placeholder numbers with empirically-measured cutoffs (real-solve sweeps + a healthy-roster baseline; evidence in `docs/prd/suspected-threshold-spike-115.md`) without reshaping the checks — raising the cross-cabin-frenemies ratio and the gender-dominance share, keeping the other three. The tiers rank proven-before-suspected; a within-tier ordering *by likelihood* is still future work.
...
The **suspected** tier ships as advisory pressure hints appended below the proven findings, each behind a conservative threshold (in `config`) that errs toward silence — none fire on comfortably-feasible input. Research spike #115 calibrated the numbers against real solves without reshaping the checks (evidence: `docs/prd/suspected-threshold-spike-115.md`):

- **Top-2 oversubscription** — a seatrade ranked first/second by far more campers than its `2 · campers_max` seats (factor `1.5`, just above the 1.17 healthy baseline; hard infeasibility only near `4.0`); names the seatrade.
- **Cabin clustering** — most of a cabin (share `0.75`, floored to `8` campers) funnels their *first* pick into one seatrade too small to hold the cohesive cabin across both its blocks; a cohesion-vs-capacity pressure distinct from the global top-2 signal. #115 found it never *proves* infeasible but is the one signal that provokes solver timeouts, so it reads as a strain hint. Names the cabin and seatrade.
- **Cross-cabin frenemies overlap** — a frenemies group spanning cabins whose size reaches this multiple (`2.0`) of the distinct seatrades they collectively rank; the proven clash only covers *same-cabin* cliques, so the cross-cabin case can only be advisory. The sharpest signal #115 found (clean flip: feasible ≤2.0, infeasible ≥2.25 — #114's placeholder `1.0` cried wolf). Names the group.
- **Gender-balance vs. same-fleet lock** — *only when the opt-in `force_same_fleet_all_week` is on*: one gender holds most cabins (share `0.9`), straining the even split the lock constrains. #115's weakest signal and top drop candidate — the lock provably can't cause infeasibility, and the placeholder `0.75` fired below the 0.875 healthy baseline. Names the imbalance.
- **Balance vs. minimum** — a *live* seatrade whose popularity only just clears its `campers_min` (factor `2.0`), so balancing cabins across blocks can split its demand below the floor in a block. #115 drop candidate — never infeasible in the sweep; the genuine case is already the proven starvation check. Names the seatrade.

The suspected hints are advisory only — they need global reasoning to prove, so they never gate the relaxation re-solve (a suspected-only diagnosis still triggers the fallback) and the UI renders them beneath the proven causes, clearly marked as maybes.
...
8. **Infeasibility: report only for now (A), diagnose later (B).** `SolverStatus` reports error state + message. *Update (#78): the "diagnose later" half shipped — structured constraint-conflict diagnostics now run as a post-mortem; see §Infeasibility Diagnosis.*
