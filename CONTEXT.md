...
- **Suspected** — a pressure signal that likely contributed but needs global reasoning to prove; advisory, never the single answer. Shipped behind **conservative placeholder thresholds** (in `config`) that err toward silence — a hint never fires on comfortably-feasible input. The tiers rank proven-before-suspected; a within-tier ordering *by likelihood* needs comparable calibrated cutoffs, so it lands with research spike #115 (which also replaces the placeholder numbers, without reshaping the checks).
...
The **suspected** tier ships as advisory pressure hints appended below the proven findings, each behind a conservative placeholder threshold (in `config`) that errs toward silence — none fire on comfortably-feasible input; research spike #115 replaces the numbers without reshaping the checks:

- **Top-2 oversubscription** — a seatrade ranked first/second by far more campers than its `2 · campers_max` seats (factor placeholder); names the seatrade.
- **Cabin clustering** — most of a cabin (share placeholder, floored to a substantial group) funnels their *first* pick into one seatrade too small to hold the cohesive cabin across both its blocks; a cohesion-vs-capacity pressure distinct from the global top-2 signal. Names the cabin and seatrade.
- **Cross-cabin frenemies overlap** — a frenemies group spanning cabins whose size reaches the distinct seatrades they collectively rank (ratio placeholder); the proven clash only covers *same-cabin* cliques, so the cross-cabin case can only be advisory. Names the group.
- **Gender-balance vs. same-fleet lock** — *only when the opt-in `force_same_fleet_all_week` is on*: one gender holds most cabins (share placeholder), straining the even split the lock constrains. Names the imbalance.
- **Balance vs. minimum** — a *live* seatrade whose popularity only just clears its `campers_min` (factor placeholder), so balancing cabins across blocks can split its demand below the floor in a block. Names the seatrade.
